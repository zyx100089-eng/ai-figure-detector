...
\begin{abstract}
The increasing capability of generative AI models raises concerns about scientific integrity, particularly the potential for fabricating experimental results presented as figures in research papers. We investigate the problem of detecting AI-generated scientific figures by constructing a dataset of 2,269 figures comprising real charts extracted from arXiv papers and fake figures produced via two distinct generation pathways: \textit{code-level fakes} (GPT-4o generating matplotlib code) and \textit{pixel-level fakes} (Stable Diffusion generating chart images directly). We evaluate three detection approaches: a fine-tuned ResNet-18 CNN, frequency-domain analysis with SVM, and statistical feature extraction with Random Forest. Our results show that while code-level fakes are relatively easy to detect (up to 100\% F1), pixel-level fakes pose a greater challenge, reducing detection performance by up to 3 percentage points. The CNN detector achieves the best overall performance (98.8\% F1), while statistical features prove most robust across fake types. Our dataset, code, and trained models are publicly available to support further research in scientific integrity verification.
\end{abstract}
...
\begin{enumerate}
    \item We construct the first dataset specifically designed for AI-generated scientific figure detection, containing 1,978 real figures from arXiv and 291 fake figures generated via two pathways (code-level and pixel-level).
...
\textbf{Pixel-Level Fakes (Track A).} We use Stable Diffusion v1.5 to generate chart images directly from text prompts describing scientific figures. This pathway bypasses code entirely, producing images through learned pixel distributions. We generate 50 figures at 512$\times$512 resolution across the same chart types.
...
Statistical + RF & 0.973 & 0.956 & 0.989 & 0.972 & 0.996 \\
CNN (ResNet-18) & 0.985 & 1.000 & 0.977 & 0.988 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}

The CNN detector achieves the best overall performance with 98.5\% accuracy and 98.8\% F1 score. The statistical feature approach performs comparably (97.2\% F1), while frequency analysis, though still effective, shows the lowest performance (93.0\% F1).
...
\textbf{Limitations.} Our dataset, while sufficient for demonstrating the detection pipeline, is limited in scale (291 fake figures). The pixel-level fakes are generated by a single model (SD v1.5); newer models like SDXL or DALL-E 3 may produce harder-to-detect figures. Additionally, our detectors are trained on ML-domain charts and may not generalize to other scientific disciplines.
...
\textbf{Future Work.} We identify several directions: (1) scaling the dataset with more generation models, (2) per-chart-type analysis of detection difficulty, (3) ensemble methods combining all three detectors, (4) cross-domain evaluation on figures from biology, physics, and chemistry papers, and (5) integration with existing scientific integrity tools.
